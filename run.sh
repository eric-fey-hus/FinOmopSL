...
# RUN SN: 
echo "Starting SN..."
#/opt/hpe/swarm-learning/scripts/bin/run-sn --name=sn-hus.org --label type=sn --hostname=sn-hus.org --dns=10.10.0.3 --sn-api-service=api.sn.hus.org.swarm --sn-p2p-service=p2p.sn.hus.org.swarm:30353 -p 30353:30303 -p 30374:30304 --sentinel --apls-ip=172.17.0.4 -e no_proxy= --socket-path /tmp/agent.sock 
/opt/hpe/swarm-learning/scripts/bin/run-sn -d --name=sn-hus.org --hostname=sn-hus.org --label type=sn --sn-api-service=api.sn.hus.org.swarm:30374 -p 30374:30304 --sn-p2p-service=p2p.sn.hus.org.swarm:30353 --apls-ip=172.17.0.4 --dns=10.10.0.3 --sentinel -e no_proxy= --socket-path /tmp/agent.sock
...
# RUN SWOP: 
echo "Starting SWOP..."
/opt/hpe/swarm-learning/scripts/bin/run-swop -d --name=swop-hus.org --label type=swop --dns=10.10.0.3 --sn-api-service=api.sn.hus.org.swarm --apls-ip=172.17.0.4 -e no_proxy= -e http_proxy= -e https_proxy= -e SWOP_KEEP_CONTAINERS=1 --usr-dir=/opt/hpe/swarm-learning/projects/FinOmopSL/swop --profile-file-name=swop_profile.yaml --socket-
path /tmp/agent.sock 


# RUN SWCI: 
/opt/hpe/swarm-learning/scripts/bin/run-swci --rm=false -dt --name=swci-hus.org --label type=swci --dns=10.10.0.3 --apls-ip=172.17.0.4 -e no_proxy= -e http_proxy= -e https_proxy= --usr-dir=/opt/hpe/swarm-learning/projects/FinOmopSL/swci --init-script-name=swci-init --socket-path /tmp/agent.sock